...
namespace vmath
{
    ...
    public:
        ...
        inline const T* raw() { return data; }
    ...
    template <typename T, int len>
    static inline std::string toString(const vecN<T, len> v)
    {
        std::stringstream ss;
        ss << v[0];
        for (int i = 1; i < len; i++)
    ...
    public:
        ...
        inline const T* raw() { return reinterpret_cast<const T*>(data); }
    ...
    static inline vecN<T, h> operator*(const matNM<T, w, h>& mat, const vecN<T, w>& that)
    {
        vecN<T, h> temp{};
