...
void main()
{
	...
	hsv.x = fract(hsv.x + atan(screenPosition.y, screenPosition.x)*0.1591549430 + exp(dist*.25+1));
...
//		// (0,0)=>white, (1,1)=>white, (0,1)=>black, (1,0)=>black
//		// (x+y) % 2 == fract((x+y)/2)
...
//		fColor = vec4(.2,.2,.2, 1) + vec4(.6, .6, .6, 1) * step(fract(white/2),0);
		fColor = vec4(.6,.6,.6,1);
	}
	/*if(color.r < 0 )
	{
